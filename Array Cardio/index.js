...
const filteredInvents = inventors.filter((inv) => inv.year >=1500 && inv.year <=1600)
console.log(filteredInvents)
...
const invWithCats = inventors.filter((inv) => inv.categories !== undefined)
const mathInv = invWithCats.filter((inv) => inv.categories.includes("mathematician"))
console.log(mathInv)
...
const invWithCats2 = inventors.filter((inv) => inv.categories !== undefined)
const mathInv2 = invWithCats2.filter((inv) => inv.categories.includes("man") && inv.categories.includes("physicist"))
console.log(mathInv2)
...
const invNames = inventors.map((inv) => `${inv.first} ${inv.last}`)
// Alternativ: const invNames = inventors.map((inv) => [inv.first, inv.last])
...
const invMails = inventors.map((inv) => `${inv.first}${inv.year}@inventor.com`.toLowerCase())
console.log(invMails)
